...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontents.com/csdfsdffese/xrayrsh/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontents.com/csdfsdffese/xrayrsh/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/csdfsdffese/xrayr ---
  ${green}0.${plain} 修改配置
————————————————
